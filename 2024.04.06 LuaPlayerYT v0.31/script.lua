...
lang = System.getLang()
if lang == 'Rus' then lang = "Russian" end
...
function held(button)
	return buttons.hold[button](pad)
end
...
local menu = {
	...
	Russian = {
		...
		{name = 'Видео', file = 'samples/video.lua', desc = 'Демонстрация воспроизведения \nвидеофайлов в формате .pmp'},
	...
	English = {
		...
		{name = 'Video', file = 'samples/video.lua', desc = 'Play a .pmp file'},
...
while true do
	...
	print(478-intraFont.textW(deFfont,tostring(System.getTime().hour..':'..menu.time)),1,colors.white,tostring(System.getTime().hour..':'..menu.time))
	
	for i, v in ipairs(menu[lang]) do
		print(50,29 + i*12,(i == menu.num and colors.blue) or colors.white,v.name)
		if menu.num == i then
			currentfile = v.file
	...
	print (260,199,colors.white,menu[lang].exit)
	print(240-intraFont.textW(deFfont, menu[lang].demo)/2,239,colors.white,menu[lang].demo)
	print(240-intraFont.textW(deFfont, menu[lang].demo2)/2,249,colors.white,menu[lang].demo2)
